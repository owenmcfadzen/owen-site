...
export default function Timeline() {
  ...
  return (
    <div style={{ margin: '32px 0', padding: '24px 0' }}>
      <div style={{ display: 'flex', position: 'relative' }}>
        <div style={{ position: 'absolute', top: 7, left: 0, right: 0, height: 1, background: 'currentColor', opacity: 0.12 }} />
        {events.map((e, i) => (
          <div key={i} onClick={() => setActive(active === i ? null : i)} style={{ flex: 1, cursor: 'pointer', textAlign: 'center', padding: '0 4px', position: 'relative' }}>
            <div style={{
            ...
            {active === i && (
              <div style={{
                ...
                marginTop: 6, opacity: 0.55, maxWidth: 110, margin: '6px auto 0',
              }}>{e.label}</div>
            )}
          </div>
        ))}
